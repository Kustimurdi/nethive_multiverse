...
function plot_acc_snapshots(event_log; t_snaps, nrows=2, ncols=3)
    ...
    hms = Makie.Plot[]  # collect heatmaps to set common color range

    # First pass: compute matrices so we can unify color scaling across all panels
    ...
    # common color limits ignoring NaNs
    allvals = reduce(vcat, [vec(m) for m in mats])
    finitevals = allvals[isfinite.(allvals)]
    clim = (minimum(finitevals), maximum(finitevals))  # or force (0,1) if accuracies
    ...
    for k in 1:length(t_snaps)
        ...
        ax = Axis(
            fig[r, c],
            xlabel = L"\text{agent id}",
            ylabel = L"\text{task id}",
            xlabelsize = labelsize,
            ylabelsize = labelsize,
            xticklabelsize = ticklabelsize,
            yticklabelsize = ticklabelsize,
            title = "t = $(round(t_snaps[k]; digits=2))"
        )

        xs = xss[k]; ys = yss[k]
        hm = heatmap!(ax, 1:length(xs), 1:length(ys), mats[k];
    ...
    # one shared colorbar (attach to the last heatmap is fine since colorrange is shared)
    Colorbar(fig[:, ncols+1], hms[end], label="acc")
...
function plot_final_across_reps(event_logs::Vector{DataFrame};
                                ncols::Int=4,
    ...
    for k in 1:n
        ...
        mat, xs, ys = acc_matrix_final(event_logs[k])

        hm = heatmap!(ax, 1:length(xs), 1:length(ys), mat;
                      colormap=:viridis, colorrange=clim)
    ...
    # global labels (optional; you can add later again)
