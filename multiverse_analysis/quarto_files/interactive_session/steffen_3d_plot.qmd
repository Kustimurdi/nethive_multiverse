...
log1 = filter(row -> row.bee1_id ==1, log)
log2 = filter(row -> row.bee1_id ==2, log)
log3 = filter(row -> row.bee1_id ==5, log)
```
...
fig = Figure(resolution=(1200,600), backgroundcolor = RGBf(0.98, 0.98, 1.0))
...
lines!(ax, log1.task_1, log1.task_2, log1.task_3; linewidth=4, color=RGBAf(0.8,0.1,0.1,0.9), label="agent 1")
lines!(ax, log2.task_1, log2.task_2, log2.task_3; linewidth=4, color=RGBAf(0.1,0.2,0.8,0.9), label="agent 2")
lines!(ax, log3.task_1, log3.task_2, log3.task_3; linewidth=4, color=RGBAf(0.1,0.7,0.2,0.9), label="agent 3")

# mark start (circle) and end (cross) for each bee
scatter!(ax, [first(log1.task_1)], [first(log1.task_2)], [first(log1.task_3)]; markersize=12, color=:black)
scatter!(ax, [last(log1.task_1)],  [last(log1.task_2)],  [last(log1.task_3)];  markersize=10, marker = :x, color=:black)

scatter!(ax, [first(log2.task_1)], [first(log2.task_2)], [first(log2.task_3)]; markersize=12, color=:black)
scatter!(ax, [last(log2.task_1)],  [last(log2.task_2)],  [last(log2.task_3)];  markersize=10, marker = :x, color=:black)

scatter!(ax, [first(log3.task_1)], [first(log3.task_2)], [first(log3.task_3)]; markersize=12, color=:black)
scatter!(ax, [last(log3.task_1)],  [last(log3.task_2)],  [last(log3.task_3)];  markersize=10, marker = :x, color=:black)
...
cam3d!(ax.scene, Vec3f(3, -4, 2), Vec3f(mean(vcat(log1.task_1, log2.task_1, log3.task_1)),
...
eye = eye_from_azel(center, r, -45, 20)
